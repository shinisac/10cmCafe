...
<div class="wrapper">
		...
		<h2 align="center">총 매출검색 결과</h2>
		<h4 align="center">${date} ~ ${date2}</h4>
		<section>
			<article>
			<form name="salesSearch" action="salesList.do" method="POST">
				<input type="hidden" name="cp" value="${cp }">
				<table border="0" cellspacing="0">
					<tr>
						<td colspan="6">
						<c:if test="${empty list}">
							<div style="float: left;"><b>검색 총 매출:0원</b></div>
						</c:if>
						<c:if test="${!empty list }">
					...
					<tfoot>
						...
						<c:if test="${empty date and empty date2}">
							...
							<c:if test="${empty list }">
								<td colspan="7" align="center">
									<a href="salesList.do?cp=${cp }">메인으로</a>
								</td>
							</c:if>
					...
					<tbody>
						...
						<c:if test="${empty list }">
							<tr>
								<td align="center" colspan="6">등록된 주문이 없습니다.</td>
							</tr>
						</c:if>
